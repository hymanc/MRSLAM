...
if makeVideo
    try
        votype = 'VideoWriter';
        vname = sprintf('video_%s', Param.dataAssociation);
        vo = VideoWriter(vname, 'MPEG-4');
...
Param.beta = [10, deg2rad(5)]; % [cm, rad]
Param.R = diag(Param.beta.^2);

% Step size between filter updates, can be less than 1.
Param.deltaT=0.1; % [s]

% Mahalanobis distance threshold for new landmark in NN
Param.NNThreshold = 150;
...
for t = 1:numSteps
    ...
    plotcov2d(muPose(1), muPose(2), SigmaPose, 'b', 0, 'b', 0, 3);
    % Landmarks
    for i = 1:State.Ekf.nL
        idx = State.Ekf.iL{i};
        lmu = State.Ekf.mu(idx);
        cov = State.Ekf.Sigma(idx,idx); 
        plotcov2d(lmu(1), lmu(2), cov, 'r', 0, 'r', 0, 3);
    end
