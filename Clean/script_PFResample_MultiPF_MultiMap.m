    ...
    if (nParticles>1 & t>1)
        ...
        for a1=1:nRobots
            ...
            if (max(weight(:,a1))>1/3)
                [robOdom(:,a1,:),map(:,:,a1,:),robPoseMapFrame(:,:,a1,:),weight(:,a1)]=resampleSingle(robOdom(:,a1,:),map,robPoseMapFrame(:,:,a1,:),weight(:,a1));
            end
            if (max(weightReverse(:,a1))>1/3)
