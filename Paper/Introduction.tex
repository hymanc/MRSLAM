...
Fig. \ref{fig:HowardFig3} depicts the process with robot 1's triple $(x^1_t,u^1_{t-1},z^1_t)$, and robot 2's triple $(x^2_t,u^2_{t-1},z^2_t)$.
...
The paper is structured as follows: In \S\ref{S:Back} some background about MRSLAM is provided, the contribution of \cite{howard2006multi} is discussed and a modified scheme is presented.  In \S\ref{S:Alg} the algorithm of \cite{howard2006multi} is discussed, and in \S\ref{S:Exp} experimental results are provided for a simulated example and the Albert B data set \cite{Radish}.
